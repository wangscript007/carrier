...
#ifndef WEBSOCKET_GATEWAY_ASYNC_HPP
#define WEBSOCKET_GATEWAY_ASYNC_HPP
...
{
    public:
        request(T& g, tcp::socket socket) : gw(g),
        ws_(std::move(socket))
        {
            setup_stream(ws_);
        }
        ...
        void run()
        {
            ...
            {
                res.set(http::field::server, std::string(BOOST_BEAST_VERSION_STRING) + " websocket-gateway-async");
            }));
...
class response : public std::enable_shared_from_this<response<T>>
{
    public:
        ...
        resolver_(net::make_strand(ioc)), ws_(net::make_strand(ioc)), service_(service)
        {
            setup_stream(ws_);
        }
        ...
        void on_connect(error_code_t ec, results_t::endpoint_type)
        {
            ...
            {
                req.set(http::field::user_agent, std::string(BOOST_BEAST_VERSION_STRING) + " websocket-gateway-async");
            }));
...
class listener : public std::enable_shared_from_this<listener>
{
    public:
        ...
        void run(const service_t& services)
        {
            ...
            {
                auto resp = std::make_shared<response<listener>>(*this, ioc_, service);
